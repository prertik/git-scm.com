...
class Book
  ...
  def absolute_path(path)
    File.absolute_path(File.join(File.dirname(__FILE__), "..", "book", @language_code, "v#{@edition}", path))
  end
  ...
  def save
    ...
    path = File.join(File.dirname(__FILE__), "..", "_data", "book-#{@language_code}.yml")
    ...
    front_matter["redirect_from"] = [ "/book/#{@language_code}" ]
    if @language_code == "en"
      front_matter["redirect_from"].append("/book")
    end

    front_matter = "#{front_matter.to_yaml}\n---"

    path = self.absolute_path("index.html")
    FileUtils.mkdir_p(File.dirname(path))
    File.open(path, 'w') do |file|
