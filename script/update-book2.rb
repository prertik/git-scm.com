...
def genbook(language_code, &get_content)
  nav = '<div id="nav"><a href="{{ page.book.section.previous | relative_url }}">prev</a> | <a href="{{ page.book.section.next | relative_url }}">next</a></div>'
  ...
  alldoc.xpath("//div[@class='sect1']").each_with_index do |entry, index|
    ...
    chapter.search("div[@class=sect2]").each do |sec|
      ...
      subsec&.each do |sub|
        sub = sub.first
        begin
          html.gsub!(/<img src="#{sub}"/, "<img src=\"{{ \"book/en/v2/#{sub}\" | relative_url }}\"")
          images.append(sub)
        rescue StandardError
          nil
        end
